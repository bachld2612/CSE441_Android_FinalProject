...
export function findDotBaoVeId(
  ...
  criteria: { hocKi?: number; namBatDau?: number; dotThu?: number; tenDotBaoVe?: string }
): number | null {
  const norm = (s: string) =>
  ...
  let filtered = list;
  if (targetName) {
    filtered = filtered.filter((d) => norm(d.tenDotBaoVe) === norm(targetName!));
  }

  if (criteria.hocKi) filtered = filtered.filter((d) => d.hocKi === criteria.hocKi);
  if (criteria.namBatDau) filtered = filtered.filter((d) => d.namBatDau === criteria.namBatDau);
...
export async function getDotBaoVeIdBy(
  criteria: { hocKi?: number; namBatDau?: number; dotThu?: number; tenDotBaoVe?: string }
): Promise<number | null> {
  const list = await getDotBaoVeList();
