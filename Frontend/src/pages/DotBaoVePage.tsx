...
export default function DotBaoVePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openAdd} className="bg-[#457B9D] text-white hover:bg-[#3b6b86]" title="Thêm đợt đồ án">
          ...
          <DialogContent className="sm:max-w-xl bg-white" aria-describedby="dotbv-desc">
            ...
            <div className="grid gap-4">
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {/* ====== Thời gian bắt đầu (icon bên phải) ====== */}
                <div className="grid gap-2">
                ...
                {/* ====== Thời gian kết thúc (icon bên phải) ====== */}
                <div className="grid gap-2">
      ...
      {/* Bảng + pagination (không hiển thị mã) */}
      <div className="bg-white rounded-xl shadow-sm">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[40%]">Tên đợt</TableHead>
              <TableHead className="w-[10%]">Học kì</TableHead>
              <TableHead className="w-[25%]">Thời gian</TableHead>
              <TableHead className="w-[15%]">Năm học</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={`${d.tenDotBaoVe}-${d.thoiGianBatDau}-${d.namBatDau}`}>
                <TableCell className="font-medium">{d.tenDotBaoVe}</TableCell>
                <TableCell>{d.hocKi}</TableCell>
                <TableCell>{d.thoiGianBatDau?.slice(0, 10)} → {d.thoiGianKetThuc?.slice(0, 10)}</TableCell>
                <TableCell>{d.namBatDau} - {d.namKetThuc}</TableCell>
                <TableCell className="text-right">
                  <Button size="icon" variant="outline" onClick={() => openEdit(d)} title="Sửa" type="button">
                    <ColoredPencil />
                  </Button>
                </TableCell>
              </TableRow>
