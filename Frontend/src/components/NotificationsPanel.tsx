...
export default function NotificationsPanel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await getThongBaoPage({ page: 0, size: 1, sort: "updatedAt,DESC" });
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await getThongBaoPage({ page: 0, size: 50, sort: "updatedAt,DESC" });
      if (!res.result) {
        ...
        <div className="fixed inset-0 z-[9999]">
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3">
              ...
              <button onClick={() => setOpen(false)} className="p-1 rounded hover:bg-gray-100" aria-label="Đóng">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="px-4">
              <div className="flex gap-3 text-sm">
                <button
                  className={`pb-2 ${activeTab === "all" ? "font-semibold border-b-2 border-black" : "text-gray-600"}`}
                  onClick={() => switchTab("all")}
                >
                  Tất cả
                </button>
                <button
                  className={`pb-2 ${activeTab === "unread" ? "font-semibold border-b-2 border-black" : "text-gray-600"}`}
                      ...
                      >
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0">
                            <div className={`text-[14px] font-medium ${isRead ? "text-gray-700" : "text-gray-900"} line-clamp-1`}>
                              {tb.tieuDe}
                            </div>
